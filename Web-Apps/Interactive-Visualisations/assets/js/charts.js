...
function updateToolTip(xProperty,yProperty, circlesGroup) {
	...
	else if (yProperty === "case_fatality_int") {
	ylabel = "Case Fatality Percentage:";
	}
	else {
...
d3.json("http://127.0.0.1:5000/").then((covidData) => {
    var data = JSON.parse(covidData).filter(function (dateobject) {
      return dateobject.date === '2022-04-19 00:00:00';
    })
	...
	d.deaths = +d.deaths;
	d.case_fatality_int = +parseInt(d.case_fatality, 10)
    });
	...
	var case_fatalityLabel = labelsGroup.append("text")
	.attr("transform","rotate(-90)")
	.attr("x", (margin.left) * 1.25)
	.attr("y", 0 - (height -60))
	.attr("value", "case_fatality_int") 
	.classed("inactive", true)
	.text("Case Fatality Percentage");
...
function getCovidCountryData(country) { 
  ...
  for (var i = 0; i < countryData.length; i++) {
	...
	hospitalisations.push(countryData[i].hospital_occupancy);
  }
  ...
  // Confirmed cases
    ...
    var layoutConfirmed = {
        ...
        xaxis: {title: {text: "Date"}, range: ['2020-01-05 00:00:00','2022-04-19 00:00:00']},
  ...
  var traceHospitalisationss = {
	...
	// Define trace data
	var traceDataHospitalisationss = [traceHospitalisationss];

	// Define layout
	var layoutHospitalisations = {
		title: "Hospitalisations over Duration of COVID-19 Pandemic",
		xaxis: {title: {text: "Date"}, range: ['2020-01-05 00:00:00','2022-04-19 00:00:00']},
		yaxis: {title: {text: "Number of Hospitalisations"}}
	};    

	// Render the plot to the div tag with id "deaths"
	Plotly.newPlot("hospitalisations", traceDataHospitalisationss, layoutHospitalisations);
	}
